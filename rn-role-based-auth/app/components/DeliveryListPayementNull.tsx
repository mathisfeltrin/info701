...
const DeliveryListPresenceFalse: React.FC<DeliveryListProps> = ({
  ...
  const fetchDeliveries = async () => {
    ...
    try {
      ...
      setDeliveries(
        data.filter((delivery: Delivery) => delivery.paid === false)
      );
  ...
  const updateDeliveryPayement = async (id: string, paid: boolean) => {
    try {
      const response = await fetch(`${deliveriesUrl}/${id}/payement`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Erreur lors de la mise à jour du payement");
      }
      ...
      setDeliveries((prevDeliveries) =>
        prevDeliveries.map((delivery) =>
          delivery._id === id ? { ...delivery, paid } : delivery
        )
      );

      console.log("Livraison mise à jour :", updatedDelivery);
      Alert.alert("Succès", "Payement mis à jour !");
    } catch (error) {
      console.error(error);
      Alert.alert("Erreur", "Impossible de mettre à jour le payement");
    }
  };

  const handleSetDeliveryStatus = (id: string) => {
    Alert.alert("Payement", "Le client a-t-il payé ?", [
      { text: "Non", onPress: () => updateDeliveryPayement(id, false) },
      { text: "Oui", onPress: () => updateDeliveryPayement(id, true) },
    ]);
  };
