...
const DeliveryListDateLivraisonNull: React.FC<DeliveryListProps> = ({
  sellerSite,
  sellerRole,
}) => {
  ...
  const updateDateLivraison = async (
    id: string,
    dateLivraison: Date | null
  ) => {
    if (!dateLivraison) {
    ...
    try {
      const response = await fetch(`${deliveriesUrl}/${id}/dateLivraison`, {
        method: "PUT",
        headers: {
      ...
      setDeliveries((prevDeliveries) =>
        prevDeliveries.map((delivery) =>
          delivery._id === id ? { ...delivery, dateLivraison } : delivery
        )
      );

      console.log("Livraison mise à jour :", updatedDelivery);
      Alert.alert("Succès", "Date de livraison mise à jour !");
      closeModal();
    } catch (error) {
      console.error(error);
      Alert.alert(
        "Erreur",
        "Impossible de mettre à jour la date de livraison."
      );
    }
  };
  ...
  const confirmDate = () => {
    if (tempDate && selectedDelivery) {
      updateDateLivraison(selectedDelivery._id, tempDate);
...
export default DeliveryListDateLivraisonNull;
